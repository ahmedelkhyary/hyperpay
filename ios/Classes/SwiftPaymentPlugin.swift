...
public class SwiftPaymentPlugin: NSObject, FlutterPlugin, SFSafariViewControllerDelegate,
    OPPCheckoutProviderDelegate
{
    ...
    let flutterChannel:String = "Hyperpay.demo.fultter/channel";
        let channel = FlutterMethodChannel(
            name: flutterChannel, binaryMessenger: registrar.messenger())
    ...
    private func openCheckoutUI(checkoutId: String, result1: @escaping FlutterResult) {
        ...
        DispatchQueue.main.async {
            ...
            let checkoutSettings = OPPCheckoutSettings()
             ...
             if(self.brandsReadyUi.contains("APPLEPAY")){
                ...
                let paymentRequest = OPPPaymentProvider.paymentRequest(
                    withMerchantIdentifier: self.applePaybundel, countryCode: self.countryCode)
                paymentRequest.paymentSummaryItems = [
                    PKPaymentSummaryItem(
                        label: self.companyName, amount: NSDecimalNumber(value: self.amount))
                ]
            ...
            self.checkoutProvider?.presentCheckout(
                forSubmittingTransactionCompletionHandler: {
        ...
        if !OPPCardPaymentParams.isNumberValid(self.number, luhnCheck: true) {
            ...
            do {
                let params = try OPPCardPaymentParams(
                    ...
                    number: self.number, expiryMonth: self.month, expiryYear: self.year,
                    cvv: self.cvv)
    ...
    private func storedCardPayment(checkoutId: String, result1: @escaping FlutterResult) {
        ...
        do {
            ...
            if self.cvv.isEmpty {
                params = try OPPTokenPaymentParams(
                    checkoutID: checkoutId, tokenID: self.tokenID, paymentBrand: self.brands)
            } else {
                params = try OPPTokenPaymentParams(
                    checkoutID: checkoutId, tokenID: self.tokenID, cardPaymentBrand: self.brands,
                    cvv: self.cvv)
            }
            params.shopperResultURL = self.shopperResultURL + "://result"
            self.transaction = OPPTransaction(paymentParams: params)
            self.provider.submitTransaction(self.transaction!) { (transaction, error) in
                guard let transaction = self.transaction else {
                    result1(
                        FlutterError(
                            code: "1", message: error?.localizedDescription ?? "Unknown error",
                            details: nil))
                    return
                }
                if transaction.type == .synchronous {
                    ...
                    NotificationCenter.default.addObserver(
                        self,
                        selector: #selector(self.didReceiveAsynchronousPaymentCallback(result:)),
                        name: Notification.Name(rawValue: "AsyncPaymentCompletedNotificationKey"),
                        object: nil)
                } else {
                    result1(
                        FlutterError(
                            code: "1", message: error?.localizedDescription ?? "Transaction failed",
                            details: nil))
                }
            }
        } catch let error as NSError {
    ...
    @objc func didReceiveAsynchronousPaymentCallback(result: @escaping FlutterResult) {
        NotificationCenter.default.removeObserver(
            self, name: Notification.Name(rawValue: "AsyncPaymentCompletedNotificationKey"),
            object: nil)
